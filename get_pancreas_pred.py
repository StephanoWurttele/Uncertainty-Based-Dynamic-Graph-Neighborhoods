...
#     i+=1
input_image = np.load("./our_pancreas_ct/pancreas_npy_3d/test/IMG-0013/256cap/IMG-0013.npy")
...
def get_border_segmentation():
    for i in range(219):
        ...
        for y in range(512):
            ...
            for x in range(512):
                if(not found):
                    if(sheet_segmentado[y][x] != 0.0):
                        found = True
                        sheet_no_segmentado[y][x] = 255
                else:
                    if(sheet_segmentado[y][x] == 0.0):
                        sheet_no_segmentado[y][x] = 255
                        break
